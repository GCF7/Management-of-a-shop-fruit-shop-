...
public class GUIBuscarPais extends JFrame {

	private String[] labels = { "Id", "Nombre" };
	//private JTextField[] texts = new JTextField[labels.length];
	private JTextField[] texts = new JTextField[2];
	
	public GUIBuscarPais(){
		super();
		initGUI();
	}
	...
	private void initGUI() {
		...
		JPanel mainPanel = new JPanel(new BorderLayout(10, 10));
			...
			northPanel.add(new JLabel("Buscar Pais"));
				...
				button.addActionListener(new ActionListener(){
					...
					public void actionPerformed(ActionEvent e) {
						...
						if (create) {
							...
							try {
							Controlador.getInstance().accion(Events.BUSCAR_PAIS, Integer.parseInt(out[0]));
							} catch (Exception ex) {
	...
	public void update (TPais pais){
		texts[0].setText("" + pais.getId());
		texts[1].setText("" + pais.getNombre());
		for (int i = 0; i < labels.length; ++i){
			if (!labels[i].equalsIgnoreCase("Id"))
