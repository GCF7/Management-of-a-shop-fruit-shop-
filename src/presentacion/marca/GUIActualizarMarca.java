...
public class GUIActualizarMarca extends JFrame {

	private String[] labels = { "Id", "IdPais", "Nombre" };
	private JTextField[] texts = new JTextField[labels.length];
	
	public GUIActualizarMarca(){
		super();
		initGUI();
	}
	...
	private void initGUI() {
		...
		JPanel mainPanel = new JPanel(new BorderLayout(10, 10));
			...
			northPanel.add(new JLabel("Actualizar Marca"));
				...
				button.addActionListener(new ActionListener(){
					...
					public void actionPerformed(ActionEvent e) {
						...
						if (create) {
							...
							try {
							Controlador.getInstance().accion(Events.ACTUALIZAR_MARCA, 
									new TMarca(Integer.parseInt(out[0]), Integer.parseInt(out[1]), true, out[2], 0));
							} catch (Exception ex) {
